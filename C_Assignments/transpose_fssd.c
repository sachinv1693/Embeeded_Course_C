...
//Function prototype	
void print_matrix(int* mat, int rows, int cols, int type_size);
...
void multiply(int** mult, int* matrix, int* matrix_tr, int rows, int cols)
{
		for (int k = 0; k < rows; k++)
		{
				...
				for (int i = 0; i < rows; i++)
				{
						for (int j = 0; j < cols; j++)
						{
								mult[k][i] += *((matrix + k * cols) + j) * *((matrix_tr + j * rows) + i);
						}
					   	printf("%5d\t", mult[k][i]);//Keeping proper spacing & 5 digits right aligned
				}
				printf("\n");
		}
}
